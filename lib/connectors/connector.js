...
export default class Connector extends EventEmitter {
  ...
  createDirectoryStructure(client, remotePaths) {
    ...
    const tld = this.connection.remote.split("/").filter(e => e);
    ...
    console.log(tld);
    console.log(directory.split("/").filter(e => e).length < tld.length);

    if (directory.split("/").filter(e => e).length < tld.length) {
